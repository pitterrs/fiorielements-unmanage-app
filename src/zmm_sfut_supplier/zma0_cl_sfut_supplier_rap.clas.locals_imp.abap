...
CLASS lcl_source_system IMPLEMENTATION.
  ...
  METHOD create_query_expression.
    ...
    " for OData V4 - Will not impact OData V2 scenarios
    ...
    IF lv_search_expression IS NOT INITIAL.
      ...
      " Create the where clause based on the fields on the interface
  ...
  METHOD get_search_fields.
    ...
    DATA(lv_search_size) =  strlen( im_search_expression ).

    LOOP AT im_screen_fields INTO DATA(lv_screen_field).

      FREE lr_field_info.

      DATA(lv_field) = |ZMA0_SFUT_SUPPLIER_IF-{ lv_screen_field }|.

      " The method call should be made with the "syntactic sugar" to
      " allow the code to retrieve exceptions when needed
  ...
  METHOD get_sorting_criteria.
    ...
    DATA(lt_sort_criteria) = VALUE string_table(
        FOR sort_element IN im_sort_elements (
            sort_element-element_name && COND #(
                WHEN sort_element-descending = abap_true
                    THEN ` descending`
                    ELSE ` ascending`
            )
        )
